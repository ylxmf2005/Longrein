---
name: explain
description: "当 AgentCorp 需要把 bug、测试进度、交付状态、评审发现、实现细节、计划或技术取舍解释给没有读过代码或产物的 sponsor/operator 时使用。用户表示看不懂、要求零上下文解释、要求通俗解释，或需要把 AgentCorp 输出翻译成清晰的零上下文语言时使用。"
---
...
目标是在不牺牲技术准确性的前提下，让解释容易跟上——而且标准是参与，不是点头：好的解释让读者有能力对*下一个*决定发表意见（修不修、发不发、选哪个方案），而不只是接受当前这个结论。适用于 bug 解释、测试进度、评审发现、交付报告、实现 walkthrough、选项解释和状态汇报。

与同类能力的分工：开工之前、还没有任何 artifact 描述的领域属于 `probe`；在合并前带理解测验（quiz）、真正吃透一整个变更属于 `walkthrough`；在 forge diff UI 里逐 hunk 的审计级注释属于 `change-detailed-walker`。你负责把某个具体的 artifact、finding、事件或状态翻译成零上下文语言——当一个解释分支/PR/diff 的请求实际上是“让我理解这个变更”时，应该推荐 `walkthrough`，而不是硬把解释集撑大。
...
多项解释时，一项一文件，类似 `review-researcher`：一条 finding、一条测试结果、一个设计选择或一个实现点各自成文。索引列出每项的一句话摘要和链接，让 sponsor 可以先扫一遍全集，而不必一次读完所有内容。
...
- 落库解释要让每个文件都自足到可以乱序阅读：重述局部背景、具体论点、证据和当前状态。
...
> 保存看起来成功了，但数据库没有任何变化。前端发送的是 `userId`，而后端只接受 `user_id`，于是后端拒绝了这次请求。页面没有把这个拒绝暴露出来，导致失败看起来像成功。
...
> 正常登录流程在浏览器里可以走通。过期会话的处理没有测试，所以这只对正常路径给出信心，对超时行为没有。

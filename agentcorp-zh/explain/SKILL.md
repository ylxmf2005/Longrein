---
name: explain
description: "当 AgentCorp 需要把 bug、测试进度、交付状态、评审发现、实现细节、计划、图或技术取舍解释给没有读过代码或产物的 sponsor/operator 时使用。用户表示看不懂、要求零上下文解释、通俗解释、低心智负担解释，不了解 repo/领域/jargon/技术背景，或需要把 AgentCorp 输出翻译成清晰中文时使用。当具体例子和 Mermaid 图能降低理解成本时主动使用。"
---
...
默认 sponsor 聪明，但缺上下文。他可能不知道 repo、领域词汇、历史决策、指标或实现约定。先讲成人话，先翻译 jargon 再使用 jargon，并先说明这件事为什么重要，再展开机制。
...
低心智负担解释通常用这个紧凑结构就够：短版、为什么重要、必要时给一个小例子、最后说明哪些是推断或仍缺失。
...
- 一次只引入一个新术语；如果必须解释多个术语，用一个小 glossary 或表格。

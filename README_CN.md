...
```text
/agentcorp:delivery-orchestrator mode:direct pace:guided effort:low 修复 config.py 的空值判断，并逐步解释
/agentcorp:delivery-orchestrator mode:partial pace:continuous effort:high 给公开 API 加限流，并在压测下验证
/agentcorp:delivery-orchestrator mode:full pace:continuous effort:max lang:zh-CN 重构三个服务间的支付回调，并验证迁移
```

不写某个参数时，交付编排器会替你推荐。只需要其中一项能力时，也可以直接调用单个技能：

```text
/agentcorp:code-review-lead depth:full 合并前对当前 diff 做一次完整代码评审
/agentcorp:parallel-researcher scope:both depth:source-verified 为这个仓库比较主流的持久化工作流引擎
/agentcorp:probe output:inline 开工前快速摸清认证模块的边界和未知项
/agentcorp:walkthrough format:html quiz:on 合并前把这个分支讲懂并考核我的理解
/agentcorp:replay session:last focus:friction output:inline 看上一轮会话反复卡在哪里
```

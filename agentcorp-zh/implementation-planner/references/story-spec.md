...
需要或有用时，参照 `templates/implementation-story-spec.demo.md`。Status 初始化为 `ready-for-plan-review`；别把你写的 Story Spec 标记为 ready to develop——这得等 Plan Review Lead 批准。
...
6. Status 是 `ready-for-plan-review`（或按协议 blocked 规则为 `blocked`），frontmatter 与 demo 一致，Review Focus 章节告诉 Plan Review Lead 该看哪里。

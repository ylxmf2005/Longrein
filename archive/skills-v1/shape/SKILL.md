...
用户允许“调查其他差异”“看看还有什么问题”只扩大证据面，不自动扩大产品范围或实现授权。调查发现新的输入类别、拒绝行为、兼容策略、公共面或长期责任时，先记录为事实、finding 或候选决定；只有用户明确选择后才进入 UD、Requirements 和 Scope。用户同时表达最小修改和一个可宽可窄的行为要求时，可以先完成不依赖歧义的收缩动作，但依赖宽解释的部分必须暂停并提出一个聚焦问题。
...
`AD-*` 只能回答已授权目标内的“怎么做”，不能同时替用户回答“要不要做”。判断一个候选 AD 时做删除检验：删除它后，当前已确认承诺若仍能成立，而该选择会新增可观察拒绝、输入类别、公共接口、共享模型传播或维护责任，它属于 scope delta，必须形成 Owner decision，不能用 AD、Completion Evidence 或“禁止静默失败”的一般原则吸收。

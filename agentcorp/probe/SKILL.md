...
- `true/false`, `yes/no`, `on/off`, and `T/F` are accepted.
- Default is `file:true` for any non-trivial probe.

When `file:true`, write or update the report path below before the final answer. When `file:false`, answer inline, but keep the same discipline: investigate first, anchor claims, include an Unknowns Ledger, and say which sources were deliberately unswept. If the conversation lasts more than one round, the sponsor asks to keep the map, or the findings become input to `brainstorm`/requirements, create the file then and continue there.

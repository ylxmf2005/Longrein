...
| "status 写着 ready_for_plan_review，说明它基本就绪了。" | 那个 status 是作者的 claim，检验它正是你的全部工作。把 status 当作不存在来读这份 spec。 |
...
- Planner 产出的 Story Spec 使用 `ready_for_plan_review`；将 approval 记录到 Plan Review Decision 中——不要改写 planner 的 status。

...
- 当你的 phase 需要读取、diff 或编辑代码时，动手前先按 assignment 的 Baseline（来自 `task.md` 的 base 分支 + merge-base commit）核对检出状态：确认自己在指定的工作分支上，且其历史包含该 merge-base。对不上时——或代码相关工作压根没拿到 baseline 时——通过 receipt（`blocked` / 证据缺口）提出来，而不是继续干：站在错误 base 上做出的工作，会让下游每一个声称都无从核验。
